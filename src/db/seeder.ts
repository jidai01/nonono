...
export async function seedTestData(database: SQLite.SQLiteDatabase) {
  ...
  const relapseNotes = [
    ...
    'Work deadline压力太大. Old habits crept in.',
  ];
  ...
  console.log(`[seeder] Seeded ${entries.length} entries (Aug 1 - Sep 12, 2026), ${activities.length} activities, ${schedules.length} schedules`);
}
